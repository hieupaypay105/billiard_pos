...
class CongTacVienEditScreen extends StatefulWidget {
  const CongTacVienEditScreen({required this.item, super.key});

  final CongTacVienItem item;

  @override
  State<CongTacVienEditScreen> createState() => _CongTacVienEditScreenState();
}

class _CongTacVienEditScreenState extends State<CongTacVienEditScreen> {
  ...
  Future<void> _onDone() async {
    ...
    final success = await provider.updatePartner(data: data);
                ...
                Container(
                  ...
                  child: FormBuilder(
                    ...
                    initialValue: {
                      ...
                      _fCode: widget.item.code,
                      _fEmail: widget.item.email,
                      _fStatus: widget.item.status == '1',
                      if (widget.item.parentId.isNotEmpty &&
                          widget.item.parentId != '0')
                        _fSaleQl: widget.item.parentId,
                    },
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        _buildFormDropdown<String>(
                          ...
                          placeholder: provider.isLoadingOptions
                              ...
                              : 'Chọn Sale quản lý',
                          items: () {
                            final currentParentId = widget.item.parentId;
                            final list = <DropdownMenuItem<String>>[];
                            var hasCurrent = false;

                            if (options != null) {
                              for (final e in options.sale.entries) {
                                if (e.key.toString() == currentParentId) {
                                  hasCurrent = true;
                                }
                                list.add(
                            ...
                            if (!hasCurrent &&
                              ...
                              list.add(
                                DropdownMenuItem<String>(
                                  ...
                                  child: Text(
                                    widget.item.saleName.isNotEmpty
                                        ? widget.item.saleName
                                        : 'Đang tải...',
                                    style: const TextStyle(
                                      fontFamily: 'Inter',
                                      fontSize: 16,
